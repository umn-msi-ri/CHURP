...
# up multiple genomes, then then should hopefully know that and be able to
# see that from the samplesheet. The genome is the 10th field, and the GTF is the 13th.
...
rnaseqc_fname <- paste(params["workdir"], "allsamples", "RNASeq_Metrics.txt", sep="/")
...
``r genome``
and the path to the GTF annotation is
``r annot``
...
library(knitr)
# Get the read files names
# Define a NA-robust function 
base_na <- function(x) {
    if(is.na(x)) {
        return("NA")
    } else {
...
names(sumsheet) <- c(
    ...
    "Trim Options",
    "Strand")
sumsheet[,1:2]                                                #Show only the name and group
```
...
## Estimated Contamination {.tabset}
...
table headers to show the summaries of rRNA contamination.
...
plotqual_first <- function(sn, draw_ax, rt) {
    ...
    image(
        ...
        axes=FALSE,
        main="Quality Key")
    axis(side=1, at=seq(0, 1, length.out=46)[c(TRUE,F,F,F,F)], 
         labels=as.character(round(seq(0,45, length.out=10))), las=1, cex.axis=0.9, tck=-0.05)
    ...
    if(draw_ax) {
        axis(side=1, at=seq(0, 1, length.out=nrow(dat))[c(TRUE, F,F,F,F)], labels=as.character(dat$Base)[c(TRUE, FALSE,F,F,F)], las=1, cex.axis=0.5, tck=-0.05)
    }
    mtext(side=2, font=2, cex=0.7, las=2, sn)
    qr2_file <- paste(params["workdir"], "singlesamples", sn, 
                      paste0(sn, "_2.",rt,"_quals.txt", sep=""), sep="/")
    if(!file.exists(qr2_file)) {
        ...
        if(draw_ax) {
            axis(side=1, at=seq(0, 1, length.out=nrow(dat))[c(T,F,F,F,F)],
                 labels=as.character(dat$Base)[c(T,F,F,F,F)], las=1, cex.axis=0.5, tck=-0.05)
    }
    }
}
...
plotqual <- function(sn,rt) {
    ...
    mtext(side=2, font=2, cex=0.7, las=2, sn)
    qr2_file <- paste(params["workdir"], "singlesamples", sn, 
                      paste0(sn, "_2.",rt,"_quals.txt"), sep="/")
    if(!file.exists(qr2_file)) {
        plot(c(0, 1), c(0, 1), ann=F, bty="n", type="n", xaxt="n", yaxt="n")
        text(x=0.5, y=0.5, "No R2", cex=1, col="black")
    } else{
...
plotqual_last <- function(sn,rt) {
    ...
    image(
        ...
        axes=FALSE)
    mtext(side=2, font=2, cex=0.7, las=2, sn)
    axis(side=1, at=seq(0, 1, length.out=nrow(dat))[c(T, F, F,F,F)], labels=as.character(dat$Base)[c(T, F,F,F,F)], las=2, cex.axis=0.9, tck=-0.05)
    qr2_file <- paste(params["workdir"], "singlesamples", sn, 
                      paste0(sn, "_2.",rt,"_quals.txt", sep=""), sep="/")
    if(!file.exists(qr2_file)) {
        plot(c(0, 1), c(0, 1), ann=F, bty="n", type="n", xaxt="n", yaxt="n")
        ...
        image(
            ...
            col=c("darkred", "indianred", "yellow", "gold", "darkgreen", "green"),
            axes=FALSE)
        axis(side=1, at=seq(0, 1, length.out=nrow(dat))[c(TRUE, FALSE,F,F,F)], labels=as.character(dat$Base)[c(TRUE, FALSE,F,F,F)], las=2, cex.axis=0.9, tck=-0.05)
    }
}

```

```{r raw_read_quals, echo=FALSE, message=FALSE, fig.width=8, fig.height=8, results="asis"}
# plotqual_first <- function(sn, draw_ax) {
#     allquals <- matrix(0:45)
#     image(
#         allquals,
#         breaks=c(0, 10, 20, 25, 30, 40, 45),
...
#         dummy <- plotqual_first(subsamp[1], one_samp)
#         if(nsubsamp > 2) {
...
#     # Set the margins of the top to be high on the first two so we can add the R1/R2 labels
#     par(mar=c(1.1, 5, 1.1, 0.1), mgp=c(1.25, 0.15, 0))
#     dummy <- plotqual_first(samplenames[1], one_samp)
#     if(nsamp > 2) {
#         par(mar=c(1.1, 5, 1.1, 0.1))
#         dummy <- sapply(samplenames[3:nsamp-1], plotqual)
#     }
#     # This looks silly, but if we have just 1 sample, then we do not need to draw the bottom labels
#     if(nsamp > 1) {
#         par(mar=c(2.5, 5, 0.1, 0.1), mgp=c(1.25, 0.15, 0))
...
```{r trim_read_quals, echo=FALSE, message=FALSE, fig.width=8, fig.height=8, results="asis"}
# if(length(trim_samp) > 0) {
#     plotqual_first <- function(sn, draw_ax) {
#         allquals <- matrix(0:45)
#         image(
...
```{r rrna_map_plot, echo=FALSE, message=FALSE, fig.width=8, fig.height=4, results="asis"}
par(mar=c(4, 4, 5, 1), mgp=c(2.5, 0.75, 0))
if(rnaseqc_do) {
...
if(large_dataset) {
    ...
    for(i in unique(chunks)) {
        ...
        at <- barplot(
            ...
            axes=FALSE)
        axis(side=2)
        axis(side=1, at=at, labels=samplenames[idx], las=2, cex.axis=0.7)
        cat("\n\n")
    }
} else {
    at <- barplot(
        ...
        axes=FALSE)
    axis(side=2)
    axis(side=1, at=at, labels=samplenames, las=2, cex.axis=0.7)
}
```
...
**Note** that the group ordering is based on alphabetical sort order. This
means that if you named your groups `Case` and `Control` then `Case` will be
the **denominator** of the comparison. This is opposite to what is
conventionally done; you must flip the signs on the log(Fold Change) values
if you want to interpret the changes in the `Case` group with respect to the
`Control` group. The P-values (both nominal and FDR-adjusted) and CPM values are
unaffected.
